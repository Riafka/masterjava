...
public class UserProcessor {
    ...
    public List<FailedEmails> process(final StaxStreamProcessor processor, Map<String, City> cities, Map<String, Group> groups, int chunkSize) throws XMLStreamException, JAXBException {
        ...
        List<UserGroup> chunkUserGroup = new ArrayList<>(chunkSize);
        ...
        while (processor.doUntil(XMLEvent.START_ELEMENT, "User")) {
            ...
            String groupRefAttr = processor.getAttribute("groupRefs");
            String[] groupRefs;
            if (groupRefAttr == null) {
                groupRefs = new String[0];
            } else {
                groupRefs = groupRefAttr.split(" ");
            }
            ...
            if (cities.get(cityRef) == null) {
            ...
            } else if (Arrays.stream(groupRefs).anyMatch(group -> groups.get(group) == null)) {
                failed.add(new FailedEmails(xmlUser.getEmail(), "Groups '" + Arrays.toString(groupRefs) + "' is not present in DB"));
            } else {
                final User user = new User(id++, xmlUser.getValue(), xmlUser.getEmail(), UserFlag.valueOf(xmlUser.getFlag().value()), cityRef);
                chunk.add(user);
                chunkUserGroup.addAll(
                        Arrays.stream(groupRefs)
                                .map(groupRef -> new UserGroup(user.getId(), groups.get(groupRef).getId()))
                ...
                if (chunk.size() == chunkSize) {
                    ...
                    chunkUserGroup = new ArrayList<>(chunkSize);
